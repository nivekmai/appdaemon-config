...
class DoorLight(hass.Hass):
    def initialize(self):
        ...
        self.log(
            "Initializing door light: {} door: {} use_sun: {} timeout: {}".format(
                self.light, self.door, self.use_sun, self.timeout
            )
        )
    ...
    def on_door_change(self, entity, attribute, old, new, kwargs):
        ...
        if self.use_sun and self.sun_down() or not self.use_sun:
            if new == "unlocked" or new == "on":
                self.log("INFO: door off (unlocked) and sun_down")
                self.turn_on(self.light)
            else:
                self.log("INFO: door on (locked) and sun_down")
                self.timer_handle = self.run_in(self.turn_off_light, self.timeout)

    def turn_off_light(self, *args):
        self.cancel_timer(self.timer_handle)
        self.turn_off(self.light)
        self.log("INFO: turning off light {}".format(self.light))
