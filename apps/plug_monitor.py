...
class BlindsControl(appapi.AppDaemon):
    def initialize(self):
    ...
    def open_blinds(self, *args):
        ...
        self.call_service("scene/turn_on", entity_id=self.open_scene)
        self.cancel_listen()
    ...
    def close_blinds(self, *args):
        ...
        for window in self.windows:
            ...
            if self.get_state(window) == "off":
                self.log(
                    "Refusing to close blinds, {} is still open".format(
                        self.friendly_name(window)
                    )
                )
                self.listen_handler = self.listen_state(
                    self.close_blinds, window, new="on"
                )
                self.log("listening for changes to {}".format(window))
                ok_to_close = False
                break
        if ok_to_close:
            self.call_service("scene/turn_on", entity_id=self.close_scene)
