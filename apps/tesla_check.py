...
class TeslaCheck(hass.Hass):
    def initialize(self):
        check_times = self.split_device_list(self.args.get("check_times", "22:00:00"))
        ...
        self.battery_sensor = self.args.get(
            "battery_sensor", "sensor.tesla_model_s_5yjsa1h20ffp78391_battery_sensor"
        )
        self.speakers = self.split_device_list(
            self.args.get(
                "speakers",
                "media_player.living_room_home,media_player.office,media_player.couch_speaker,media_player.master_bedroom_home",
            )
        )
        self.charge_level = int(self.args.get("charge_level", 50))
        self.car_name = self.args.get("name", "Model S")
        self.log(
            "Tesla check init: {}".format(
                json.dumps(
                    {
