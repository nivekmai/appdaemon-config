...
class LightFlux(hass.Hass):
    ...
    def parse_times(self, *args):
        ...
        self.reset_time = datetime.combine(
            date.today(), self.parse_time(self.reset_time_str)
        )
    ...
    def calculate_dim(self):
        ...
        level = (
            self.start_level
            - int(
                delta_from_start.total_seconds()
    ...
    def set_dim(self, *args):
        ...
        self.log(
            "setting {} to {}, state: {}, level: {}".format(
                light, self.level, current_setting, current_brightness
            )
        )
