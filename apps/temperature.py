...
class TemperatureChecker(hass.Hass):
    def initialize(self):
        ...
        self.log("Initializing temperature checker: mode: {} target: {} windows: {} temperature: {}"
                 .format(self.mode, self.target, self.windows, self.temperature))
    ...
    def check_heat(self):
        ...
        if mode == "heat" and window == "on" and target >= temperature:
            self.log("INFO: mode: {} and window {} and {} > {}".format(mode, window, target, temperature))
            return True
        return False
    ...
    def check_should_warn(self):
        ...
        if overheating and not self.disabling:
            self.say_on_speakers('Warning! Heater on and windows open. Close the windows or thermostat will be disabled in 5 minutes.')
            self.disabling = True
        ...
        if overcooling and not self.disabling:
            self.say_on_speakers('Warning! Air conditioning on and windows open. Close the windows or thermostat will be disabled in 5 minutes.')
            self.disabling = True
    ...
    def check_should_disable(self, *args):
        if self.check_cool():
            self.say_on_speakers('We can not cool the planet, turning off the air conditioning.')
            self.call_service('climate/set_temperature', entity_id=self.ac, temperature=90)
        if self.check_heat():
            self.say_on_speakers('We can not warm the planet, turning off the heater.')
            self.call_service('climate/set_temperature', entity_id=self.ac, temperature=50)
        self.disabling = False
