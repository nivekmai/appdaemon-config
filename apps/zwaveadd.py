...
class ZwaveAdd(hass.Hass):
  def initialize(self):
    ...
    self.log("Initializing motion sensor light: {} motion: {} timeout: {}".format(self.light, self.motion, self.timeout))
  ...
  def on_timeout(self, *args):
    ...
    if self.get_state(self.motion) is "on":
      self.log("{} is still detecting movement (state: {}), not turning off the light".format(self.motion, self.get_state(self.motion)))
      self.timeout_handler = self.run_in(self.on_timeout, self.timeout)
    else:
